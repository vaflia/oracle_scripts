...
-- выбираем документы для импорта      
    ...
    COMMIT;
    /*---------------------------------------------------------------------------------------------*/
    -- ЗАГРУЗКА СЧЕТЧИКОВ ОДПУ ЧЕРЕЗ ПАКЕТ P_METER2
 BEGIN   
 IF P_var= 4 THEN 
          L_parent_ml :=null;
          l_id_meter   :=null;
          ...
          DELETE  FROM ORALV.T_DOC WHERE fk_doctp in (select id from oralv.t_doc_tp where cd in('Итоговый реестр ОДПУ','Счет-фактура'));
-- выбираем документы для импорта      
   FOR rec_t_doc in (select * from oralv.t_doc@hotora where coalesce(fk_time,0) not in ('17') and fk_doctp in (select id from oralv.t_doc_tp@hotora where cd in('Итоговый реестр ОДПУ','Счет-фактура')) and id =264243 order by dt1) 
   LOOP  
             k:=0;  
             /*вставка документа в новую базу с возвратом id */
            INSERT INTO ORALV.T_DOC 
             ...
             --выбираем все сведения по логическому и физическому счетчику для послед вставки по пакету p_meter2
                ...
                --                     oralv.t_addr_tp tp
                                        WHERE  
                                            ...
                                            and  m.fk_meter_log = ml.id
                                            and  m.id = MV.FK_METER
                                            and  mv.id=14027722--rec_mv.id--14031561
                                            and  ch.reu = ch_hot.reu
                                            and  ch.kul = ch_hot.kul
                                            and  ch.nd = ch_hot.nd
                                            and  gw.reu= ch.reu
                   ...
                   -- узнаем есть ли фэйковый счетчик на доме по услуге и вводу.
                    ...
                    IF l_parent_ml IS null THEN 
                           ...
                           (
                              ...
                              , P_PARENT_ID    => null
                              , P_NAME => 'Фэйковый счетчик по услуге- '||rec_attr.usl||' и вводу-'||rec_attr.vvod   -- на нем будут сидеть счетчики ОДПУ    !!ИЛИ  счетчики ИПУ  (так как ОДПУ может не быть)
                              , P_IS_UPDATEBLE => 0                 
                    ...
                    END IF;         
           /*------------------------------------------------------------*/
              --2 - вставка физ счетчика  если он есть psch=1,3 или даже если нету                    
                    IF ir =0  THEN
                       ...
                       IF ir<>0 then add_log ('ERRcode - '||SQLCODE||' -ERRmsg- '||SQLERRM||'.' ||rec_attr.nd_klsk,sysdate,i,369, ir); END IF;     
                         ...
                         IF ir = 0 and rec_attr.sch in (1,5) THEN  
                            ...
                            , P_IS_UPDATEBLE => 1, 
                              P_IS_COMMIT => 1);
                            IF ir<>0 then  add_log ('ERRcode - '||SQLCODE||' -ERRmsg- '||SQLERRM||'.' ||rec_attr.nd_klsk,sysdate,i,386,ir); END IF;
                         ...
                         ELSIF ir=0 and rec_attr.sch in (0) then
                             oralv.p_meter2.U_METER_EXS_INS_UPD 
                               ( P_IR => ir
                                , P_ID => l_id_exs
                                , P_FK_METER => l_id_meter
                                , P_DT1 => rec_attr.dat_NACH/*case  when coalesce(rec_kart.psch_dtg,trunc(add_months(sysdate,-1),'mm'))>trunc(add_months(sysdate,-1),'mm')      
                                                            then trunc(add_months(sysdate,-1),'mm') 
                                                            else trunc(add_months(sysdate,-1),'mm')  
                                                   end*/  
                                , P_DT2 =>rec_attr.dat_end-- last_day(add_months(trunc(sysdate,'dd'),-1))
                                , P_IS_UPDATEBLE => 1, 
                                  P_IS_COMMIT => 1);
                         END IF;  
                    END IF;      
               ...
               -- узнаем есть ли фэйковый счетчик на доме по услуге и вводу.
                    ...
                    IF l_parent_ml is null THEN 
                           ...
                           (
                              ...
                              , P_DT2 => to_date('20990101', 'YYYYMMDD')
                              , P_PARENT_ID => null
                              , P_NAME => 'Фэйковый счетчик по услуге ГВ и вводу-'||rec_kart.vvod_gw   -- на нем будут сидеть счетчики ОДПУ    !!ИЛИ  счетчики ИПУ  (так как ОДПУ может не быть)
                              , P_IS_UPDATEBLE =>1                 
                              , P_IS_COMMIT => 1 --  -1 - вызов для проверки существования, без DML (и без COMMIT)
                                                              --   0 - DML без COMMIT
                                                              --   1 - DML и COMMIT
               ...
               -- узнаем есть ли фэйковый счетчик на доме по услуге и вводу.
                    ...
                    IF l_parent_ml is null THEN 
                           ...
                           (
                              ...
                              , P_DT2 => to_date('20990101', 'YYYYMMDD')
                              , P_PARENT_ID => null
                              , P_NAME => 'Фэйковый счетчик по услуге ХВ и вводу-' || rec_kart.vvod  -- на нем будут сидеть счетчики ОДПУ    !!ИЛИ  счетчики ИПУ  (так как ОДПУ может не быть)
                              , P_IS_UPDATEBLE =>1                 
                              , P_IS_COMMIT => 1 --  -1 - вызов для проверки существования, без DML (и без COMMIT)
                                                              --   0 - DML без COMMIT
                                                              --   1 - DML и COMMIT
               ...
               -- узнаем есть ли фэйковый счетчик на доме по услуге и вводу.
                    ...
                    IF l_parent_ml is null THEN 
                           ...
                           (
                              ...
                              , P_DT2 => to_date('20990101', 'YYYYMMDD')
                              , P_PARENT_ID => null
                              , P_NAME => 'Фэйковый счетчик по услуге ЭЛ.ЭН и вводу-'||rec_kart.vvod_el   -- на нем будут сидеть счетчики ОДПУ    !!ИЛИ  счетчики ИПУ  (так как ОДПУ может не быть)
                              , P_IS_UPDATEBLE =>1                 
                              , P_IS_COMMIT => 1 --  -1 - вызов для проверки существования, без DML (и без COMMIT)
                                                              --   0 - DML без COMMIT
                                                              --   1 - DML и COMMIT
...
BEGIN
 --   IF C1%FOUND THEN 
    FOR REC IN C1 LOOP
        p_parent_ml := rec.id;
  --      p_k_lsk_obj  := rec.fk_k_lsk;
    END LOOP;
  --  ELSE p_parent_ml :=NULL;
 --            p_k_lsk_obj :=NULL;
  --  END IF;
END;
-- -------------------
