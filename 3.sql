...
SELECT tp.name_tr,tp.trest,tp.reu,h.ska "Чистая",h.pn "Пеня",h.bn_summa "вт.ч. безнал", h.ska+h.pn "Сумма общая с пеней",  
            p.sumplan,
...
FROM   
    ...
    (SELECT h.reu,sum(h.ska) ska,sum(h.pn) pn,
                 sum(case when l_pay= 1 then h.ska+h.pn else 0 end) AS BN_SUMMA 
...
-- сводный запрос для OLAP
SELECT dtek,h.name,tp.trest,tp.reu,h.ska,h.pn,h.bn_ska,h.ska+h.bn_ska S_ALL,  
            p.sumplan,
            coalesce(p.sumplan,0)*1000-coalesce(h.ska,0) as plan_no,
            CASE WHEN p.sumplan<>0 THEN ROUND((coalesce(h.ska,0))/10/p.sumplan,2) ELSE 0 END as procplan
FROM   
