declare
  ...
  select usl,f_ras_v,usl_norm,f_ras_izmvol_subsid
  from usl u
  ...
  f_ras_izmvol_subsid_ usl.f_ras_izmvol_subsid%type;
  mg_ params.period%type;
  time_ date;
BEGIN
    --alter table scott.info_usl_lsk truncate partition MG201402;
    --alter table scott.info_usl_nd truncate partition MG201402;
 /*------------------------------------------------------------------------------------------*/
 ...
 select '201401' into mg_ from dual;
  --select to_char(add_months(sysdate,-1),'yyyymm') into mg_ from dual;
 --update prep.load_kartw201401 k set k.nabor_id = (select t.nabor_id from kart t where t.lsk = k.lsk);
 /*execute immediate 'drop table scott.prepload_kartw';
  execute immediate 'create table scott.prepload_kartw as select * from prep.load_kartw201401';
...
/* UPDATE scott.prepload_kartw SET gw_nor=
 ...
 --конец апдейтов
   ...
   --и temp_stat c детализацией до usl а не uslm как в статистике
    ...
    loop    
      fetch r
        into usl_, f_ras_v_, usl_norm_, f_ras_izmvol_subsid_;
...
WHERE usl in ('009','010','017','018','060','054','055','057');
    ...
    ( select lsk,usl,org  from t_volume_usl
        union all
        select lsk,usl,org  from prep.t_charges_usl201401
        where usl in (select usl from usl where f_ras_v is not null)
        union all
        select lsk,usl,org  from prep.t_changes_usl201401
        where usl in (select usl from usl where f_ras_v is not null)
        union all
        select lsk,usl,org  from prep.t_privs_usl201401
        where usl in (select usl from usl where f_ras_v is not null)
        union all
        select lsk,usl,org  from prep.t_payment_usl201401
        where usl in (select usl from usl where f_ras_v is not null)
    )
  GROUP BY lsk,usl,org;


--  DELETE FROM scott.info_usl_lsk t where mg='201401';-- and reu='K1';
              -- 004   007-ОТ
            -- 006   011-ХВ
            -- 008   015-ГВ
  ...
  FROM (SELECT t.*, reu, kul, nd, kw, psch, sch_el, vvod, vvod_gw, vvod_ot, s.id 
         ...
         prep.t_charges_usl201401 cr,
         prep.t_changes_usl201401 cn,
         (select lsk,usl,org,sum(summa) summa, sum(decode(type,0,0,null,0,summa)) summa_cn from prep.t_privs_usl201401 group by lsk,usl,org) pr,
         scott.t_volume_usl vl,
         prep.t_payment_usl201401 pm, usl u
   WHERE 
          h.lsk=cr.lsk(+)
     -- and h.reu='K1'    
      and h.usl=cr.usl(+)
...
-- проставляем признаки счетчиков ОДПУ
     MERGE INTO scott.info_usl_lsk i
                using ( select lg.*,u.usl from
                                 (SELECT distinct reu,kul,nd,vvod,coalesce(sch,0) as sch, 
                                  ...
                                  FROM ldo.load_gw t1
                                WHERE '201401' between dat_nach and dat_end) lg , scott.usl u
                                where lg.uslm=u.uslm ) lg
                 ON  
                (
                      i.reu=lg.reu and i.kul=lg.kul and i.nd=lg.nd 
                  and i.vvod_gw=lg.VVOD and i.usl=lg.usl and i.mg='201401'-- and i.reu in('K1')
                )
  ...
  FOR rec in
   ...
   LOOP
    ...
    'UPDATE scott.info_usl_lsk krt SET psch=nvl('||rec.s_ras_sch||',0)
     ...
     USING rec.usl, '201401' ;
   END LOOP;
   commit;

 
-- DELETE FROM scott.info_usl_nd t where mg='201401'; 
 ...
 SELECT  t.reu, t.kul, t.nd, t.usl, t.org, t.vvod_gw, sum(t.charges), sum(t.changes), sum(t.privs) , sum(t.payment), sum(t.volume), t.mg,
  ...
  FROM   scott.info_usl_lsk t, 
          ( select lsk, org, usl, sum(vol) as vol_izm,sum(gkal) as gkal_izm from scott.t_volume_usl_izm 
             where mg='201401' group by lsk, org, usl ) izm
  WHERE     t.mg = '201401'
         and t.lsk  = izm.lsk (+)
         and t.org = izm.org  (+)
         and t.usl  = izm.usl (+)
